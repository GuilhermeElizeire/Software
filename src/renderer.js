// Aguardamos o DOM estar pronto para evitar erros de elementos nulos
document.addEventListener('DOMContentLoaded', async () => {
    
    // 1. Verificação de segurança para as APIs do Tauri 2.0
    if (!window.__TAURI__) {
        console.error("Erro: APIs do Tauri não encontradas. Verifique se 'withGlobalTauri' está true no tauri.conf.json");
        return;
    }
    ...
    // 2. Escuta o evento vindo do motor Rust (mDNS)
    await listen('dispositivo-encontrado', (event) => {
        const dados = event.payload; 
    ...
    // --- FUNÇÕES GLOBAIS (Atribuídas ao window para funcionar no HTML) ---
    ...
    window.acionarRele = async (ip, cleanId) => {
        ...
        btn.style.background = "#0af31d"; 

        try {
            await fetch(`http://${ip}/rele`, { method: 'POST', mode: 'no-cors' });
            btn.innerText = "✅ Sucesso"; 
            btn.style.background = "#283fa7"; 
        } catch (e) {
            btn.innerText = "❌ Falha"; 
